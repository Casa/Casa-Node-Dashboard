...
const populate = {
  async status() {
    ...
    if(status) {
      ...
      if(!status.unlocked && status.operational && !LightningData.isUnlocking) {
      ...
      if(LightningData.operational && LightningData.percent !== '1.0000') {
    ...
    if(LightningData.operational && LightningData.unlocked) {
      ...
      if(lightning) {
        ...
        if(lightningInfo.uris.length) {
          ...
          LightningData.address.code = lightningInfo.identityPubkey + '@' + externalIP.externalIP;
          LightningData.address.autoGenerated = false;
        }

        LightningData.address.ip = externalIP.externalIP;
...
function LightningSetup(scope) {
  ...
  EventBus.$on('load-lightning-stats', (options = {autoupdate: true}) => {
    ...
    if(options.autoupdate) {
      IntervalBus.set(populate.channels, 60);
    }
  });
